use {
  ...
  libipld::{cid::Cid, store::StoreParams},
  libp2p::request_response::{Codec as RequestResponseCodec, ProtocolName},
  std::{
    convert::TryFrom,
    io::{self, Write},
    marker::PhantomData,
  ...
  thiserror::Error,
  unsigned_varint::{aio, io::ReadError},
};
...
impl<P: StoreParams> RequestResponseCodec for BitswapCodec<P> {
  ...
  {
    let msg_len =
      u32_to_usize(aio::read_u32(&mut *io).await.map_err(|e| match e {
        ReadError::Io(e) => e,
        err => other(err),
      })?);
    ...
    let request =
      BitswapRequest::from_bytes(&self.buffer).map_err(invalid_data)?;
    Ok(request)
  }
    ...
    T: AsyncRead + Send + Unpin,
  {
    ...
    io.read_exact(&mut self.buffer).await?;
    let response =
      BitswapResponse::from_bytes(&self.buffer).map_err(invalid_data)?;
    Ok(response)
  }
    ...
    T: AsyncWrite + Send + Unpin,
  {
    ...
    let msg_len =
      unsigned_varint::encode::u32(self.buffer.len() as u32, &mut buf);
...
fn invalid_data<E: std::error::Error + Send + Sync + 'static>(
  e: E,
) -> io::Error {
...
pub(crate) mod tests {
  use {
    super::*,
    libipld::multihash::{Code, MultihashDigest},
  };
